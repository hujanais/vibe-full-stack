...
- **GET /api/v1/job**  
  ...
  Query Parameters:  
  - `state` (optional): filter by rocket state (Preparing, Ready, InFlight, Landed)  
  - `status` (optional): filter by job status (idle, running, succeeded, failed, cancelled)  
  - `user_id` (optional): filter by user who created the job  
  Response: List of jobs

- **GET /api/v1/rocket/{id}**  
  Get rocket by ID.  
  Response: Job object as above.

- **DELETE /api/v1/rocket/{id}**  
  Delete rocket by ID.  
  Response: 204 No Content
...
- **PATCH /api/v1/job/{id}**  
  ...
  ```json
  {
    ...
    "estimated_time": "integer_seconds",
    "status": "string",
    "updated_at": "this should just be autogenerated when called and can be done in code"
  }
  ```  
  Response: Updated job object.

- **GET /api/v1/flight/history/{id}**  
...
|-----------------|------------|----------------------------------------------|
| id              | UUID       | Primary key                                  |
| state           | Enum       | Rocket state: Preparing, Ready, InFlight, Landed, RUD (Rapid Unscheduled Destruction) |
...
| status          | Enum       | Job status: idle, running, succeeded, failed, cancelled |
...
## JobHistory (optional) Contains the flight history of a rocket_id
| Field           | Type       | Description                                  |
|-----------------|------------|----------------------------------------------|
| id              | UUID       | Primary key                                  |
| rocket_id       | UUID       | Foreign key to Rocket                        |
...
## Airflow Job
- The airflow job is to run the task for updating the state, location and estimated_time of a Rocket.
- The airflow pod when queried should return relevant information like {state, estimated_time, location} object.

# Notes
- Rocket also acts as Rocket inventory; only rockets in proper states (e.g., Ready) can be launched.
